...
- Implemented cumulative percentage using running_total / grand_total.
...
Window function usage: 4.7/5  
Query correctness: 4.8/5  
...
WITH customer_revenue AS(
    ...
    SUM(o.total_amount) AS daily_customer_revenue
    FROM orders o 
    JOIN customers c ON o.customer_id = c.customer_id
    WHERE o.status = 'completed'
    GROUP BY o.customer_id, c.full_name, date(o.order_date)
)
SELECT
customer_id,
full_name,
...
SUM(daily_customer_revenue) over(PARTITION BY customer_id ORDER BY date) AS running_total
